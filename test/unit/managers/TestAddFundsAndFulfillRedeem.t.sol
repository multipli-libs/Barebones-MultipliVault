...
contract TestAddFundsAndFulfillRedeem is VaultFundManagerBase {
    ...
    function setUp() public override {
        ...
        depositForUser(users.bob, INITIAL_DEPOSIT - 50_000e6); //50,000e6
    ...
    function test_AddFundsAndFulfillRedeem_RevertsWithInsufficientContractBalanceInVaultFundManager() public {
        uint256 contractBalance = usdc.balanceOf(address(fundManager));
    ...
    function test_AddFundsAndFulfillRedeem_Success() public {
        ...
        uint256 initialUserBalance = usdc.balanceOf(users.bob);
        uint256 initialVaultBalance = usdc.balanceOf(address(vault));
        uint256 initialFundManagerBalance = usdc.balanceOf(address(fundManager));
        ...
        assertEq(usdc.balanceOf(users.bob), initialUserBalance + assetsWithFee - 1e6, "bob's balance mismatch"); // Minus withdrawal fee

        // Check vault balance increased
        assertEq(usdc.balanceOf(address(vault)), initialVaultBalance, "vault's balance mismatch");
    ...
    function test_AddFundsAndFulfillRedeem_PartialFulfillment() public {
        ...
        uint256 bobUSDCBalanceBefore = usdc.balanceOf(users.bob);
        ...
        assertEq(usdc.balanceOf(users.bob), bobUSDCBalanceBefore + partialAssetsWithoutFee);
    ...
    function test_AddFundsAndFulfillRedeem_RevertsWithInvalidSharesAmount() public {
        ...
        uint256 excessiveShares = shares + 1000e6;
    ...
    function test_AddFundsAndFulfillRedeem_RevertsWithInvalidAssetsAmount() public {
        ...
        uint256 excessiveAssets = assetsWithFee + 1000e6;
    ...
    function test_AddFundsAndFulfillRedeem_MaintainsCorrectBalances() public {
        ...
        uint256 assetsAfterFee = assetsWithFee - 1e6; // Withdrawal fee is 1 USDC
        ...
        assertEq(usdc.balanceOf(address(vault)), initialVaultBalance, "vault's usdc balance mismatch");
    ...
    function test_AddFundsAndFulfillRedeem_WithDifferentReceivers() public {
        uint256 shares = 8000e6;
        uint256 assetsWithFee = vault.convertToAssets(shares);

        // Alice creates redemption request but Bob will receive the funds
        createRedemptionRequest(users.alice, shares);

        uint256 initialBobBalance = usdc.balanceOf(users.alice);
    ...
    function test_AddFundsAndFulfillRedeem_CorrectReceiver() public {
        ...
        uint256 initialAliceBalance = usdc.balanceOf(users.alice);
        ...
        uint256 expectedReceived = assetsWithFee - 1e6; // Minus 1 USDC withdrawal fee
        assertEq(usdc.balanceOf(users.alice), initialAliceBalance + expectedReceived);
    }
    ...
    function test_addFundsAndFulfillRedeem_SharePriceChangesWhenAggregatedBalanceIsUpdated() public {
        ...
        uint256 totalAssets = vault.totalAssets(); // 150_000e6
        uint256 totalSupply = vault.totalSupply(); // 150_000e6

        uint256 newAggregatedBalance = aggregatedBalance + 30_000e6; // 80_000e6 -> increases the share price to 1.2
        
        // assume that you have added 30_000e6 in yield to the vault
        // this will lead to vault getting paused
        updateUnderlyingBalance(newAggregatedBalance); // 80_000e6
        unpauseVault();
        vm.roll(block.number + 1);

        // sanity checks to ensure the state of the vaults
        assertEq(vault.totalAssets(), 180_000e6, "totalAssets mimatch");
        assertEq(vault.totalSupply(), 150_000e6, "totalSupply mismatch");
        assertEq(vault.lastPricePerShare(), 12e17, "last price mismatch"); // 1.2 USDC


        // Create redemption request
        // price per share = 1.2
        // amount to be paid out including fee = 6000e6-1 USDC (note that `1` here is not fee)
        uint256 shares = 5000e6;

        uint256 expectedPendingAssets = vault.convertToAssets(shares); // 5_999_999_999
        assertEq(expectedPendingAssets, 6000e6-1, "expectedPendingAssets mismatch"); // 5_999_999_999
        ...
        // aggregateBalance will now be 95_000e6
        // this will bring the totalAsset value to 195_000e6
        // and price price will be 1.3 USDC
        updateUnderlyingBalance(newAggregatedBalance + 15_000e6); 
        unpauseVault();
        vm.roll(block.number + 1);

        assertEq(vault.lastPricePerShare(), 13e17, "last price mismatch after update"); // 1.3 USDC
        assertEq(vault.totalAssets(), 195_000e6, "totalAssets mimatch after update");
        assertEq(vault.totalSupply(), 150_000e6, "totalSupply mismatch after update");

        uint256 initialLastPricePerShare = vault.lastPricePerShare(); // 1.3 USDC

        // redeem 5000e6 shares
        bytes memory data =
        ...
        assertEq(vault.totalAssets(), 195_000e6 - pendingAssets, "totalAssets mimatch after redemption"); // 189_000_000_001
        assertEq(vault.totalSupply(), 145_000e6, "totalSupply mismatch after redemption");
