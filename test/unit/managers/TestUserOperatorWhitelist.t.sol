...
contract TestUserOperatorWhitelist is BaseTest {
    ...
    uint256 INITIAL_OPERATOR_DEPOSIT_AMOUNT = getQuantizedValue(100);
    ...
    function setUp() public override {
        ...
        deal(address(token), operatorOwner, getQuantizedValue(100_000));
        vm.label(operatorOwner, "operatorOwner");

        operator = new MockOperator();
        deal(address(operator), 10e18);
        deal(address(token), address(operator), getQuantizedValue(1_000_000));
        ...
        deal(address(token), address(fundManager), getQuantizedValue(1_000_000));
