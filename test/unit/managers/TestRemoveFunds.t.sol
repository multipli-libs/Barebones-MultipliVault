...
contract TestRemoveFunds is VaultFundManagerBase {
    ...
    function setUp() public override {
        ...
        // Fund the fund manager contract with USDC for testing
    ...
    function test_RemoveFunds_RevertsCalledByUserAuthorizedToCallManageButUnAuthorisedToCallRemoveFunds() public {
        uint256 transferAmount = 30_000e6;
        uint256 initialContractBalance = usdc.balanceOf(address(fundManager));
        uint256 initialRecipientBalance = usdc.balanceOf(recipient1);
        ...
        // Check balances
        assertEq(usdc.balanceOf(address(fundManager)), initialContractBalance);
        assertEq(usdc.balanceOf(recipient1), initialRecipientBalance);
    }
    ...
    function test_RemoveFunds_Success() public {
        ...
        // Check balances
        assertEq(usdc.balanceOf(address(fundManager)), initialContractBalance - transferAmount);
        assertEq(usdc.balanceOf(recipient1), initialRecipientBalance + transferAmount);
    }

    function test_RemoveFunds_EmitsCorrectEvent() public {
        uint256 transferAmount = 25_000e6;
    ...
    function test_RemoveFunds_WithMaximumAmount() public {
        ...
        assertEq(usdc.balanceOf(address(fundManager)), 0);

        // Recipient should receive full amount
        assertEq(usdc.balanceOf(recipient1), initialRecipientBalance + contractBalance);
    }

    function test_RemoveFunds_MultipleTransfers() public {
        uint256 firstTransfer = 20_000e6;
        uint256 secondTransfer = 15_000e6;
        uint256 initialContractBalance = usdc.balanceOf(address(fundManager));
        ...
        // Check final state
        assertEq(usdc.balanceOf(recipient1), firstTransfer);
        assertEq(usdc.balanceOf(recipient2), secondTransfer);
        assertEq(usdc.balanceOf(address(fundManager)), initialContractBalance - firstTransfer - secondTransfer);
    }
    ...
    function test_RemoveFunds_WithDifferentInitiators() public {
        ...
        assertEq(usdc.balanceOf(recipient2), transferAmount);
    }

    function test_RemoveFunds_DoesNotAffectVaultBalances() public {
        uint256 transferAmount = 40_000e6;
    ...
    function test_RemoveFunds_WithExactBalance() public {
        ...
        assertEq(usdc.balanceOf(address(fundManager)), 0);
        assertEq(usdc.balanceOf(recipient1), contractBalance);
    }
    ...
    function test_RemoveFunds_SmallAmount() public {
        ...
        assertEq(usdc.balanceOf(recipient1), smallAmount);
    }

    function test_RemoveFunds_LargeAmount() public {
        // First add more funds to the contract
        uint256 largeAmount = 500_000e6; // 500K USDC
        deal({token: address(usdc), to: address(fundManager), give: largeAmount, adjust: true});

        uint256 transferAmount = 400_000e6; // 400K USDC
        uint256 initialContractBalance = usdc.balanceOf(address(fundManager));
