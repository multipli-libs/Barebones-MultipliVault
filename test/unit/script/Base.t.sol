...
contract BrokenAssetScript is BaseDeployment {
    function setDeploymentConfig() public override {
        OWNER = msg.sender;
        ASSET = address(0); 
        INITIAL_LOCK_DEPOSIT_AMOUNT = 100e6;
        SHARE_NAME = "xUSDC";
        SHARE_SYMBOL = "xUSDC";
        MIN_DEPOSIT_AMOUNT = 10e6;
...
abstract contract DeployXUSDCBaseTest is Test, Constants {
    ...
    uint256 constant EXPECTED_MIN_DEPOSIT_AMOUNT = 10e6; // 10 USDC

    function _initializeVault(bool isMock, string memory mainnetBaseRPCUrl, address mainnetBaseTokenAddress) internal {TOKEN_ADDRESS = mainnetBaseTokenAddress;

        vm.setEnv("IS_TEST", "true");

        if (!isMock) {
            uint256 forkId = vm.createFork(mainnetBaseRPCUrl);
        ...
        if (!isMock) {
            deal(TOKEN_ADDRESS, owner, 1000e6); // Fund owner on fork
        }
    ...
    function testRevertsIfAssetIsZero() public {
        ...
        BrokenAssetScript script = new BrokenAssetScript();
   ...
   function testValidateDeploymentConfigDirectly() public {
    ...
    function testDeployWithBroadcast_False() public {
        ...
        mockToken.mint(deployer, 1_000e6);
