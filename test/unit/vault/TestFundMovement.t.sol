...
contract TestFundMovement is BaseTest {
    ...
    function setUp() public override {
        ...
        // Fund the vault with some tokens for testing removeFunds
        vm.startPrank(users.alice);
        token.approve(address(depositVault), getQuantizedValue(10_000));
        depositVault.deposit(getQuantizedValue(10_000), users.alice);
        vm.stopPrank();
    }
    ...
    function test_removeFunds_Success_ValidTransfer() public {
        uint256 transferAmount = getQuantizedValue(1000);
        ...
        vm.expectEmit(true, true, true, true);
        emit FundMovementHelperUpgradeable.FundsRemoved(users.admin, address(token), transferAmount, recipient1);

        depositVault.removeFunds(transferAmount, recipient1);

        // Check balances
        assertEq(
            token.balanceOf(address(depositVault)), vaultBalanceBefore - transferAmount, "Vault balance should decrease"
        );
        assertEq(
            token.balanceOf(recipient1), recipientBalanceBefore + transferAmount, "Recipient balance should increase"
        );
    ...
    function test_removeFunds_Success_TransferFullBalance() public {
        ...
        assertEq(token.balanceOf(address(depositVault)), 0, "Vault should have zero balance");
        assertEq(
            token.balanceOf(recipient1), recipientBalanceBefore + vaultBalance, "Recipient should receive full amount"
        );
    ...
    function test_removeFunds_Success_MultipleTransfers() public {
        uint256 transferAmount = getQuantizedValue(500);
        ...
        uint256 vaultBalanceInitial = token.balanceOf(address(depositVault));
        ...
        assertEq(token.balanceOf(recipient1), transferAmount, "Recipient1 should receive correct amount");
        assertEq(token.balanceOf(recipient2), transferAmount, "Recipient2 should receive correct amount");
    ...
    function test_removeFunds_RevertsWhen_InsufficientBalance() public {
        uint256 vaultBalance = token.balanceOf(address(depositVault));
        uint256 excessiveAmount = vaultBalance + getQuantizedValue(1000);
    ...
    function test_Integration_WhitelistAndRemoveFunds_Workflow() public {
        uint256 transferAmount = getQuantizedValue(2000);
        ...
        assertEq(token.balanceOf(address(depositVault)), vaultBalanceBefore - transferAmount);
    ...
    function test_Integration_MultipleRecipientsManagement() public {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            assertEq(token.balanceOf(recipients[i]), balanceBefore + transferAmount);
        }
    ...
    function test_EdgeCase_WhitelistSelfAsRecipient() public {
        ...
        uint256 balanceBefore = token.balanceOf(users.admin);
        uint256 transferAmount = getQuantizedValue(1000);

        depositVault.removeFunds(transferAmount, users.admin);

        assertEq(token.balanceOf(users.admin), balanceBefore + transferAmount);
    ...
    function test_EdgeCase_VaultAsRecipient() public {
        ...
        assertEq(token.balanceOf(address(depositVault)), vaultBalanceBefore);
    ...
    function testFuzz_removeFunds_ValidAmounts(uint256 amount) public {
        ...
        assertEq(token.balanceOf(recipient1), recipientBalanceBefore + amount);
