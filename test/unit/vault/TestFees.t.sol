...
contract TestVaultFeeOperations is BaseTest {
    ...
    uint256 internal constant FLAT_FEE_AMOUNT = 100e6; // 100 USDC flat fee
    ...
    // Test amounts
    uint256 internal constant DEPOSIT_AMOUNT_SMALL = 1000e6; // 1,000 USDC
    uint256 internal constant DEPOSIT_AMOUNT_MEDIUM = 10000e6; // 10,000 USDC
    uint256 internal constant DEPOSIT_AMOUNT_LARGE = 100000e6; // 100,000 USDC
    ...
    function _fundUsers() internal {
        // Fund users
        deal(address(usdc), alice, 1000000e6); // 1M USDC
        deal(address(usdc), bob, 1000000e6); // 1M USDC
        deal(address(usdc), charlie, 1000000e6); // 1M USDC

        require(usdc.balanceOf(alice) == 1000000e6, "Pre-requisite balance check failed"); // 1M USDC
        require(usdc.balanceOf(bob) == 1000000e6, "Pre-requisite balance check failed"); // 1M USDC
        require(usdc.balanceOf(charlie) == 1000000e6, "Pre-requisite balance check failed"); // 1M USDC
    }
    ...
    function _registerAssetWithFees(IVariableVaultFee.AssetFeeConfig memory config) internal {
        vm.startPrank(users.admin);
    ...
    function test_deposit_Success_WithFlatFee() public {
        IVariableVaultFee.AssetFeeConfig memory config = _setupFlatFeeConfig();
        _registerAssetWithFees(config);
        ...
        // Sanity checks
        {
            require(aliceUsdcBefore == 1_000_000e6, "Alice balance does not match expectation");
            require(feeRecipientBefore == 0, "feeRecipientBefore balance does not match expectation");
            require(vaultUsdcBefore == 0, "Vault balance does not match expectation");
        }

        vm.startPrank(alice);
        usdc.approve(address(depositVault), depositAmount);
        ...
        // Post-state checks
        assertEq(usdc.balanceOf(alice), aliceUsdcBefore - depositAmount, "Alice should pay full deposit amount");
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBefore + expectedFee, "Fee recipient should receive fee");
        assertEq(
            usdc.balanceOf(address(depositVault)),
            vaultUsdcBefore + expectedAssetsAfterFee,
        ...
        assertEq(depositVault.balanceOf(alice), 9_900e6, "Alice should receive shares");
    ...
    function test_deposit_Success_WithPercentageFee() public {
        ...
        // sanity check
        assertEq(expectedFee, 99009901); // rounded up from 99009900.99009901
        ...
        assertEq(sharesReceived, 9_900_990_099); // depositAmount - fee = 10_000e6 - 99009901
    ...
    function test_deposit_Success_WithZeroFee() public {
        IVariableVaultFee.AssetFeeConfig memory config = _setupPercentageFeeConfig(0, 0, 0, 0);
        _registerAssetWithFees(config);
        ...
        // Sanity check for shares received
        assertEq(sharesReceived, 10_000e6); // depositAmount - fee = 10_000e6

        // No fee should be charged
        assertEq(usdc.balanceOf(alice), aliceUsdcBefore - depositAmount, "Alice should pay deposit amount");
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBefore, "No fee should be charged");
        assertEq(usdc.balanceOf(address(depositVault)), depositAmount, "Vault should receive full amount");
    }
    ...
    function test_deposit_Reverts_WithZeroAmount() public {
        ...
        assertEq(usdc.balanceOf(alice), aliceUsdcBefore, "No change in amount");
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBefore, "No fee should be charged");
        assertEq(usdc.balanceOf(address(depositVault)), 0, "No funds added to the vault");
    }
    ...
    function test_deposit_RevertsWhen_VaultPaused() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), DEPOSIT_AMOUNT_MEDIUM);
    ...
    function test_deposit_RevertsWhen_InsufficientAllowance() public {
        ...
        usdc.approve(address(depositVault), DEPOSIT_AMOUNT_MEDIUM - 1);
    ...
    function test_mint_Success_WithFlatFee() public {
        ...
        assertEq(usdc.balanceOf(alice), aliceUsdcBefore - assetsUsed, "Alice should pay correct amount");
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBefore + expectedFee, "Fee recipient should receive fee");

        assertEq(usdc.balanceOf(address(depositVault)), vaultBalanceBefore + expectedAssets - expectedFee);
        assertEq(usdc.balanceOf(address(depositVault)), 0 + 1100e6 - 100e6); // sanity check for above statement
    }
    ...
    function test_mint_Success_WithPercentageFee() public {
        ...
        // sanity checks
        assertEq(previewAssets, sharesToMint + expectedFee); // 5025e6
        assertEq(previewAssets, 5025e6);

        uint256 aliceUsdcBefore = usdc.balanceOf(alice);
        uint256 feeRecipientBefore = usdc.balanceOf(feeRecipient);

        vm.startPrank(alice);
        usdc.approve(address(depositVault), previewAssets);
        ...
        assertEq(usdc.balanceOf(address(depositVault)), vaultBalanceBefore + previewAssets - expectedFee);
        assertEq(usdc.balanceOf(address(depositVault)), 0 + 5025e6 - 25e6); // sanity check for above statement
    }
    ...
    function test_mint_RevertsWhen_VaultPaused() public {
        ...
        vm.startPrank(alice);
        vm.expectRevert(abi.encodeWithSignature("EnforcedPause()"));
        depositVault.mint(1000e6, alice);
        vm.stopPrank();
    }
    ...
    function test_requestRedeem_RevertsWhen_InsufficientShares() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), DEPOSIT_AMOUNT_SMALL);
    ...
    function test_fulfillRedeem_Success_WithFlatFee() public {
        ...
        deal(address(usdc), address(depositVault), expectedAssetsWithFee);
        ...
        assertEq(usdc.balanceOf(alice), aliceUsdcBefore + expectedAssetsWithoutFee, "Alice should receive assets");
        assertEq(
            usdc.balanceOf(feeRecipient),
            feeRecipientBefore + expectedFee,
    ...
    function test_fulfillRedeem_Success_PartialFulfillment() public {
        ...
        deal(address(usdc), address(depositVault), assetsToFulfill);
    ...
    function test_cancelRedeem_Success_PartialCancel() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), DEPOSIT_AMOUNT_LARGE);
    ...
    function test_flatWithdrawalFee_unchangedDuringPendingRedeem_usesOriginalFeeAtFulfillment() public {
        // Setup: 0 deposit fee, 100 USDC withdrawal fee
        ...
        uint256 expectedAssetsWithoutFee = depositVault.previewRedeem(shares); // Should be 200 - 100 = 100 USDC
        uint256 expectedAssetsWithFee = depositVault.convertToAssets(shares);
        ...
        // Fee remains unchanged (100 USDC) - no update needed
        ...
        // Verify: Alice gets assets minus current fee (100 USDC)
        uint256 currentFee = 100e6;
        uint256 assetsToAlice = expectedAssetsWithFee - currentFee;

        assertEq(
            usdc.balanceOf(alice), aliceBalanceBefore + assetsToAlice, "Alice should receive assets minus current fee"
        );
        assertEq(
            usdc.balanceOf(feeRecipient), feeRecipientBalanceBefore + currentFee, "Fee recipient should get current fee"
        );
    }

    function test_flatWithdrawalFee_increasedFrom100To200USDC_duringPendingRedeem_usesNewFeeAtFulfillment() public {
        // Setup: 0 deposit fee, 100 USDC withdrawal fee
        IVariableVaultFee.AssetFeeConfig memory initialConfig = IVariableVaultFee.AssetFeeConfig({
            depositFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 0}),
            withdrawalFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 100e6}),
            instantWithdrawalFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 100e6}),
            flashRedeemFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 100e6}),
            feeRecipient: feeRecipient
        });
        _registerAssetWithFees(initialConfig);
        ...
        // User initiates requestRedeem (with 100 USDC fee)
        uint256 expectedAssetsWithoutFee = depositVault.previewRedeem(shares);
        ...
        (uint256 pendingAssets, uint256 pendingShares) = depositVault.pendingRedeemRequest(alice);
        assertEq(pendingAssets, 200e6);
        assertEq(pendingShares, 200e6);

        // Update withdrawal fee to 200 USDC
        IVariableVaultFee.AssetFeeConfig memory updatedConfig = IVariableVaultFee.AssetFeeConfig({
            depositFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 0}),
            withdrawalFee: IVariableVaultFee.FeeConfig({
                feeType: IVariableVaultFee.FeeType.FLAT,
                feeAmount: 200e6 // Updated to 200 USDC
            }),
            instantWithdrawalFee: IVariableVaultFee.FeeConfig({
                feeType: IVariableVaultFee.FeeType.FLAT,
                feeAmount: 200e6 // Updated to 200 USDC
        ...
        vm.startPrank(users.admin);
        feeContract.updateAssetFeeConfig(address(usdc), updatedConfig);
        vm.stopPrank();
        ...
        deal(address(usdc), address(depositVault), expectedAssetsWithoutFee + fee);

        vm.startPrank(users.admin);
        depositVault.fulfillRedeem(alice, shares, expectedAssetWithFee);
        vm.stopPrank();

        // Verify: Alice gets assets minus NEW fee (200 USDC)
        ...
        assertEq(usdc.balanceOf(alice), aliceBalanceBefore + assetsToAlice, "Alice should receive assets minus NEW fee");
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBalanceBefore + newFee, "Fee recipient should get NEW fee");
    }

    function test_flatWithdrawalFee_increasedFrom100To250USDC_duringPendingRedeem_fulfillmentFailsWhenFeeExceedsStoredAssets(
    ) public {
        // Setup: 0 deposit fee, 100 USDC withdrawal fee
        IVariableVaultFee.AssetFeeConfig memory initialConfig = IVariableVaultFee.AssetFeeConfig({
            depositFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 0}),
            withdrawalFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 100e6}),
            instantWithdrawalFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 200e6}),    
            flashRedeemFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 200e6}),    
            feeRecipient: feeRecipient
        });
        _registerAssetWithFees(initialConfig);
        ...
        // Update withdrawal fee to 250 USDC
        IVariableVaultFee.AssetFeeConfig memory updatedConfig = IVariableVaultFee.AssetFeeConfig({
            depositFee: IVariableVaultFee.FeeConfig({feeType: IVariableVaultFee.FeeType.FLAT, feeAmount: 0}),
            withdrawalFee: IVariableVaultFee.FeeConfig({
                feeType: IVariableVaultFee.FeeType.FLAT,
                feeAmount: 250e6 // Updated to 250 USDC
            }),
            instantWithdrawalFee: IVariableVaultFee.FeeConfig({
                feeType: IVariableVaultFee.FeeType.FLAT,
                feeAmount: 350e6 // Updated to 350 USDC
            }),
            flashRedeemFee: IVariableVaultFee.FeeConfig({
                feeType: IVariableVaultFee.FeeType.FLAT,
                feeAmount: 450e6 // Updated to 450 USDC
            }),
            feeRecipient: feeRecipient
        });
        ...
        // This should revert because 250 USDC fee > 100 USDC stored assets
        ...
        public
    {
        ...
        vm.startPrank(users.admin);
        feeContract.registerAsset(address(usdc), initialConfig);
        vm.stopPrank();
        ...
        assertEq(usdc.balanceOf(feeRecipient), feeRecipientBalanceBefore + expectedFee, "Fee recipient should get fee");
    }
        ...
        public
    {
        ...
        // User deposits 10,000 USDC and requests redeem
        uint256 depositAmount = 10000e6;
        vm.startPrank(alice);
        usdc.approve(address(depositVault), depositAmount);
        uint256 shares = depositVault.deposit(depositAmount, alice);
        ...
        vm.startPrank(bob);
        usdc.approve(address(depositVault), depositAmount * 3);
        ...
        assertEq(
            usdc.balanceOf(feeRecipient), feeRecipientBalanceBefore + newFee, "Fee recipient should get NEW 1% fee"
        );
    }
    ...
    function test_deposit_Revert_VerySmallAmount() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), smallAmount);
    ...
    function test_deposit_EdgeCase_ExactFlatFeeAmount() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), exactFeeAmount);
        ...
        assertEq(usdc.balanceOf(feeRecipient), FLAT_FEE_AMOUNT, "Fee recipient should get full amount");
    }
    ...
    function test_deposit_EdgeCase_FeeGreaterThanDepositFeeAmount() public {
        ...
        assertEq(shares, 0, "Should get zero shares when deposit equals flat fee");
        assertEq(usdc.balanceOf(feeRecipient), 0, "Fee recipient should have 0 balance");
    }
    ...
    function test_mint_EdgeCase_MaximumPercentageFee() public {
        ...
        assertEq(usdc.balanceOf(feeRecipient), expectedFee, "Fee should be approximately 5%");
    }
    ...
    function test_fulfillRedeem_EdgeCase_ExactBalance() public {
        ...
        if (vaultBalance < expectedAssetsWithFee) {
            deal(address(usdc), address(depositVault), expectedAssetsWithFee - vaultBalance);
        }
    ...
    function test_feeConsistency_RoundTripNoProfit() public {
        ...
        uint256 expectedAssets = depositVault.previewRedeem(shares);
        deal(address(usdc), address(depositVault), expectedAssets);
        ...
        assertLt(finalBalance, aliceUSDCBalanceBefore, "Round trip should result in loss due to fees");
    }
    ...
    function test_integration_MultiUserWithDifferentFees() public {
        ...
        // Bob mints
        vm.startPrank(bob);
        uint256 bobShares = 5000e6;
        uint256 bobAssets = depositVault.previewMint(bobShares);
        usdc.approve(address(depositVault), bobAssets);
    ...
    function test_errorHandling_FeeContractNotSet() public {
        ...
        bytes memory data =
            abi.encodeWithSelector(MultipliVault.initialize.selector, usdc, users.admin, "Test Vault", "TEST");
        ...
        vm.startPrank(alice);
        usdc.approve(address(newVault), DEPOSIT_AMOUNT_MEDIUM);
    ...
    function test_errorHandling_AssetNotRegistered() public {
        ...
        // Should revert because USDC is not registered
        vm.expectRevert();
        depositVault.deposit(DEPOSIT_AMOUNT_MEDIUM, alice);
        vm.stopPrank();
    }
    ...
    function testFuzz_deposit_ValidAmounts(uint256 amount) public {
        amount = bound(amount, FLAT_FEE_AMOUNT + 1, 1000000e6); // Min above flat fee, max 1M USDC

        IVariableVaultFee.AssetFeeConfig memory config = _setupFlatFeeConfig();
        _registerAssetWithFees(config);

        vm.startPrank(alice);
        usdc.approve(address(depositVault), amount);
    ...
    function testFuzz_mint_ValidShares(uint256 shares) public {
        ...
        deal(address(usdc), alice, previewAssets);
