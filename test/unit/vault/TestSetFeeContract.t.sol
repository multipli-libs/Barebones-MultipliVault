...
contract TestSetFeeContract is BaseTest {
    ...
    function _registerAssetWithFees(VariableVaultFee feeContractToUse, IVariableVaultFee.AssetFeeConfig memory config)
        internal
    {
        vm.startPrank(users.admin);
        feeContractToUse.registerAsset(address(usdc), config);
        vm.stopPrank();
    }
    ...
    function test_setFeeContract_NewContractUsedForCalculations_FlatToPercentage() public {
        ...
        uint256 initialFeeRecipientBalance = usdc.balanceOf(primaryFeeRecipient);
        ...
        uint256 secondaryFeeRecipientBalanceBefore = usdc.balanceOf(secondaryFeeRecipient);

        uint256 newShares = depositVault.deposit(DEPOSIT_AMOUNT_MEDIUM, bob);
        vm.stopPrank();

        // Verify new fee calculation
        uint256 secondaryFeeRecipientBalanceAfter = usdc.balanceOf(secondaryFeeRecipient);
        ...
        assertEq(
            usdc.balanceOf(primaryFeeRecipient), FLAT_FEE_AMOUNT, "Primary fee recipient should not receive new fees"
        );
    }
    ...
    function test_setFeeContract_NewContractUsedForCalculations_PercentageToFlat() public {
        ...
        assertEq(usdc.balanceOf(primaryFeeRecipient), expectedInitialFee, "Initial percentage fee should be charged");
        ...
        uint256 actualFeeCharged = usdc.balanceOf(secondaryFeeRecipient) - secondaryFeeRecipientBalanceBefore;
    ...
    function test_setFeeContract_NewContractUsedForCalculations_DifferentPercentages() public {
        ...
        assertEq(usdc.balanceOf(primaryFeeRecipient), initialFee, "Initial 1% fee should be charged");
        ...
        assertEq(usdc.balanceOf(secondaryFeeRecipient), expectedHighFee, "Should charge 5% fee");
    ...
    function test_setFeeContract_PreviewFunctionsUpdateImmediately() public {
        ...
        vm.startPrank(alice);
        usdc.approve(address(depositVault), testAmount);
    ...
    function test_setFeeContract_CanRecoverFromZeroAddress() public {
        ...
        IVariableVaultFee.AssetFeeConfig memory config = _setupFlatFeeConfig(primaryFeeRecipient);
        _registerAssetWithFees(primaryFeeContract, config);
    ...
    function test_setFeeContract_NewContractWithoutAssetRegistration() public {
        // Switch to a fee contract that doesn't have USDC registered
        VariableVaultFee unregisteredFeeContract = new VariableVaultFee(users.admin);
        // Note: Not registering USDC in this contract
        ...
        vm.expectRevert(); // Should revert because USDC is not registered
        depositVault.deposit(DEPOSIT_AMOUNT_MEDIUM, alice);
        vm.stopPrank();
    }
    ...
    function test_setFeeContract_MultipleUsersBeforeAndAfterChange() public {
        ...
        assertGt(usdc.balanceOf(primaryFeeRecipient), 0, "Primary fee recipient should have received fees");
        assertGt(usdc.balanceOf(secondaryFeeRecipient), 0, "Secondary fee recipient should have received fees");
    ...
    function test_setFeeContract_CompleteWorkflowWithFeeContractChanges() public {
        ...
        assertEq(usdc.balanceOf(primaryFeeRecipient), FLAT_FEE_AMOUNT, "Primary should have flat fee");
        assertGt(usdc.balanceOf(secondaryFeeRecipient), 0, "Secondary should have 1% fee");
        assertGt(usdc.balanceOf(tertiaryFeeRecipient), 0, "Tertiary should have 5% fee");
    }
    ...
    function test_getFeeRecipient_UpdatesWithFeeContract() public {
        ...
        IVariableVaultFee.AssetFeeConfig memory config1 = _setupFlatFeeConfig(primaryFeeRecipient);
        _registerAssetWithFees(primaryFeeContract, config1);

        IVariableVaultFee.AssetFeeConfig memory config2 =
            _setupPercentageFeeConfig(PERCENTAGE_FEE_1_PERCENT, PERCENTAGE_FEE_1_PERCENT, secondaryFeeRecipient);
        _registerAssetWithFees(secondaryFeeContract, config2);
    ...
    function testFuzz_setFeeContract_FeeCalculationConsistency(uint256 depositAmount) public {
        depositAmount = bound(depositAmount, 1000e6, 100000e6); // 1k to 100k USDC

        // Setup two different fee contracts
        IVariableVaultFee.AssetFeeConfig memory config1 =
            _setupPercentageFeeConfig(PERCENTAGE_FEE_1_PERCENT, PERCENTAGE_FEE_1_PERCENT, primaryFeeRecipient);
        _registerAssetWithFees(primaryFeeContract, config1);
    ...
    function test_setFeeContract_PendingRedeemsWithFeeContractChange() public {
        ...
        vm.startPrank(users.admin);
        deal(address(usdc), address(depositVault), initialPendingAssets);
    ...
    function test_setFeeContract_InteractionWithVaultPauseUnpause() public {
        ...
        assertGt(usdc.balanceOf(secondaryFeeRecipient), 0, "New fee recipient should receive fees");
    }
    ...
    function test_setFeeContract_StateConsistencyAfterMultipleChanges() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            IVariableVaultFee.AssetFeeConfig memory config = _setupPercentageFeeConfig(
                PERCENTAGE_FEE_1_PERCENT + (i * PERCENTAGE_FEE_1_PERCENT), // 1%, 2%, 3%
                PERCENTAGE_FEE_1_PERCENT + (i * PERCENTAGE_FEE_1_PERCENT),
                feeRecipients[i]
            );
            _registerAssetWithFees(contracts[i], config);
        }
        ...
        for (uint256 cycle = 0; cycle < 3; cycle++) {
            for (uint256 i = 0; i < 3; i++) {
                ...
                uint256 preview = depositVault.previewDeposit(1000e6);
