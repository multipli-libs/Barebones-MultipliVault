...
contract TestPreviewFlashRedeem is BaseTest {
    ...
    uint256 DEPOSIT_AMOUNT_MEDIUM;
    uint256 DEPOSIT_AMOUNT_SMALL = 1;

    function setUp() override public {
        super.setUp();

        DEPOSIT_AMOUNT_MEDIUM = getQuantizedValue(100);
    ...
    function test_previewFlashRedeem_isSuccess_OnMediumDepositAmount() public {
        ...
        if(config.tokenConfig.decimals == 6){
            assertEq(expectedFee, 995025, "expected fee mismatch"); // (200e6 * 5e15) / (1e18 + 5e15) = 995024.875621890548
        } else if(config.tokenConfig.decimals == 8){
            assertEq(expectedFee, 99502488, "expected fee mismatch"); // (200e8 * 5e15) / (1e18 + 5e15) = 99502487.5621890548
        } else if(config.tokenConfig.decimals == 18){
            assertEq(expectedFee, 995024875621890548, "expected fee mismatch"); // (200e18 * 5e15) / (1e18 + 5e15) = 995024875621890548
        } else{
            revert('Condition not added for given decimals');
        }

        assertEq(assetAmount, getQuantizedValue(200), "expected asset amount mismatch"); // verify share price = 2
    ...
    function test_previewFlashRedeem_Success_WhenAssetsEqualFee() public {
        // Before we start the test, ensure the price of 1 share = 1 token (this is for the ease of testing)
        // remove token from vault
        moveAssetsFromVault(IERC20(token).balanceOf(address(depositVault)));
        // update underlying assets to make sure that the price of share = 1
        updateUnderlyingBalance(depositVault.totalSupply());

        uint256 shares = 1;

        // ensure the price of 1 share is 1 token
